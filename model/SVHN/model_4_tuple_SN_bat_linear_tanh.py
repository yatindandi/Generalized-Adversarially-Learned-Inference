...
class Discriminator(nn.Module):
    ...
    def forward(self, x1, z1, x2, z2):
        batch_size = x1.size()[0]
        output_x_1 = self.infer_x(x1)
        output_z_1 = self.infer_z(z1.view(batch_size,-1))
        output_x_2 = self.infer_x(x2)
        output_z_2 = self.infer_z(z2.view(batch_size,-1))
        output_features = self.infer_joint(torch.cat([output_x_1.view(batch_size,-1), output_z_1,output_x_2.view(batch_size,-1), output_z_2], dim=1))
        output = self.final(output_features)
        #if self.output_size == 1:
            #output = F.sigmoid(output)
        #else:
        #    output = F.softmax(output)
        return output.squeeze(), output_features.view(x1.size()[0], -1)
