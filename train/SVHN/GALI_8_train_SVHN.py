...
from model_4_tuple_SN_bat_linear_tanh import *
import os
...
for epoch in range(start_epoch,num_epochs+1):
    ...
    for (data, target) in train_loader:
        ...
        output_real, _ = netD(d_real + noise1, output_z.view(batch_size, latent_size, 1, 1),recon_real + noise2 ,output_z_recon_real.view(batch_size, latent_size, 1, 1))
        noise1 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        output_real_2, _ =  netD(d_real + noise1,output_z_recon_real.view(batch_size, latent_size, 1, 1),recon_real + noise2 ,output_z.view(batch_size, latent_size, 1, 1))
        noise1 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        output_real_3, _ =  netD(recon_real + noise2, output_z.view(batch_size, latent_size, 1, 1),d_real + noise1,output_z_recon_real.view(batch_size, latent_size, 1, 1))
        noise1 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        output_real_4, _ =  netD(recon_real + noise2,output_z_recon_real.view(batch_size, latent_size, 1, 1),d_real + noise1,output_z.view(batch_size, latent_size, 1, 1))
        noise1 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))        
    #output_real_i_1, _ =  netD(d_real + noise1, output_z.view(batch_size, latent_size, 1, 1),d_real + noise1, output_z_recon_real.view(batch_size, latent_size, 1, 1))
        #output_real_i_2, _ =  netD(d_real + noise1, output_z_recon_real.view(batch_size, latent_size, 1, 1),d_real + noise1, output_z.view(batch_size, latent_size, 1, 1))
        enc_fake, _, _, _  = netE(d_fake)
        enc_fake = enc_fake.view(batch_size, -1)
        #[:, :latent_size]#mu_recon_real + epsilon * sigma_recon_real
        mu_fake, log_sigma_fake = enc_fake[:, :latent_size], enc_fake[:, latent_size:]
        sigma_fake = torch.exp(log_sigma_fake)
        epsilon = Variable(tocuda(torch.randn(batch_size, latent_size)))

        enc_fake = output_z_fake = mu_fake + sigma_fake*epsilon
        recon_fake = netG(output_z_fake.view(batch_size, latent_size, 1, 1))
        
        output_fake, _ = netD(d_fake + noise1, z_fake, recon_fake + noise2, enc_fake.view(batch_size, latent_size, 1, 1))
        noise1 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        output_fake_2, _ = netD(d_fake + noise2,enc_fake.view(batch_size, latent_size, 1, 1) , recon_fake + noise1, z_fake)
        noise1 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        output_fake_3, _ = netD(recon_fake + noise1, z_fake, d_fake + noise2, enc_fake.view(batch_size, latent_size, 1, 1))
        noise1 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        output_fake_4, _ = netD(recon_fake + noise2,enc_fake.view(batch_size, latent_size, 1, 1) , d_fake + noise1, z_fake)
        ...
        loss_d = (criterion_D(o_real, real_label_1) + criterion_D(o_real2, real_label_2)  + criterion_D(o_real3, real_label_3) + criterion_D(o_real4, real_label_4)+ criterion_D(o_fake, fake_label_1) + criterion_D(o_fake2, fake_label_2) + criterion_D(o_fake3, fake_label_3) + criterion_D(o_fake4, fake_label_4))
         

        loss_g = (criterion(o_real, real_label_2) + criterion(o_real, real_label_3) + criterion(o_real, real_label_4) + criterion(o_real, fake_label_1) + criterion(o_real, fake_label_2) + criterion(o_real, fake_label_3) + criterion(o_real, fake_label_4) +\
