...
import torchvision.utils as vutils
from model_disc_4_SN_bat_linear_tanh_extra import *
#import os
...
for epoch in range(start_epoch,num_epochs+1):
    ...
    for (data, target) in train_loader:
        ...
        y_real = _infer(path_to_checkpoint_file, F.interpolate((1+d_real)/2,54))
        output_real, _ = netD(d_real + noise1, output_z.view(batch_size, latent_size, 1, 1), y_real.view(batch_size, 3072, 1, 1))
        ...
        y_recon = _infer(path_to_checkpoint_file, F.interpolate((1+recon_real)/2,54))
        output_real_2, _ = netD(d_real + noise1, output_z_recon_real.view(batch_size, latent_size, 1, 1),y_recon.view(batch_size, 3072, 1, 1))
        y_fake = _infer(path_to_checkpoint_file, F.interpolate((1+d_fake)/2,54))
        output_fake, _ = netD(d_fake + noise2, z_fake, y_fake.view(batch_size, 3072, 1, 1))
        noise2 = Variable(tocuda(torch.Tensor(data.size()).normal_(0, 0.1 * (num_epochs - epoch) / num_epochs)))
        ...
        output_fake_2, _ = netD(recon_fake + noise2, z_fake, y_fake.view(batch_size, 3072, 1, 1))
